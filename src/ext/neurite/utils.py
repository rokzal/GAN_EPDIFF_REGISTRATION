...
# Disclaimer
# Codigo de voxelmorph, mas eficiente, valorar si usar/ reimplementar.
#
@tf.function
def interpn_vox(vol, loc, interp_method='linear', fill_value=None):
    """
...
def resize(vol, zoom_factor, interp_method='linear'):
    ...
    return ne.interpn(vol, grid, interp_method=interp_method)
...
def perlin_vol(vol_shape,
    ...
    interp_order: interpolation (upscale) order, as used in ne.zoom
...
def flatten_axes(x, axes):
    ...
    Example:
        ...
        ne.flatten_axes(v, [1, 2]).shape # returns TensorShape([3, 20, 6])
...
#fftshift = tf.spectral.fftshift
#ifftshift = tf.spectral.ifftshift
