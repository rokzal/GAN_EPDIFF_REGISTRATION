...
# local imports
import src.ext.neurite.utils as ne
from src.ext.voxelmorph import layers
...
def value_at_location(x, single_vol=False, single_pts=False, force_post_absolute_val=True):
    ...
    fn = lambda y: ne.interpn(y[0], y[1])
...
def transform(vol, loc_shift, interp_method='linear', indexing='ij', fill_value=None):
    ...
    assert loc_shift.shape[-1] == nb_dims, \
        'Dimension check failed for ne.transform(): {}D volume (shape {}) called ' \
        'with {}D transform'.format(nb_dims, vol.shape[:-1], loc_shift.shape[-1])

    # location should be mesh and delta
    mesh = ne.volshape_to_meshgrid(loc_volshape, indexing=indexing)  # volume mesh
    ...
    # test single
    return ne.interpn(vol, loc, interp_method=interp_method, fill_value=fill_value)
...
def rescale_dense_transform(transform, factor, interp_method='linear'):
    ...
    def single_batch(trf):
        if factor < 1:
            trf = ne.resize(trf, factor, interp_method=interp_method)
            trf = trf * factor
        else:
...
def point_spatial_transformer(x, single=False, sdt_vol_resize=1):
    ...
    fn = lambda x: ne.interpn(x[0], x[1])
...
def affine_to_dense_shift(matrix, shape, shift_center=True, indexing='ij'):
    ...
    mesh = ne.volshape_to_meshgrid(shape, indexing=indexing)
    ...
    flat_mesh = [ne.flatten(f) for f in mesh]
